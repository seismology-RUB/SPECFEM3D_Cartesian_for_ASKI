          ...
          if (ispec_is_elastic(ispec)) then
            ...
            if (USE_FORCE_POINT_SOURCE) then
              ...
              if (USE_RICKER_TIME_FUNCTION) then
                ...
                stf = comp_source_time_function_gauss(time_source_dble,hdur_gaussian(isource)) !! FS FS replaced 5.d0*DT by hdur_gaussian(isource) (as was comitted to devel branch by D.Komatitsch on 3 Sep 2015)
              endif
          ...
          if (ispec_is_elastic(ispec)) then
            ...
            if (USE_FORCE_POINT_SOURCE) then
              ...
              if (USE_RICKER_TIME_FUNCTION) then
                ...
                ! use a very small duration of 5*DT to mimic a Dirac in time
                stf = comp_source_time_function_gauss(dble(NSTEP-it)*DT-t0-tshift_src(isource),hdur_gaussian(isource)) !! FS FS replaced 5.d0*DT by hdur_gaussian(isource) (as should have been committed to devel branch by D.Komatitsch on 3 Sep 2015)
              endif
        ...
        if (USE_FORCE_POINT_SOURCE) then
          if (USE_RICKER_TIME_FUNCTION) then
            ...
            stf_pre_compute(isource) = &
                 comp_source_time_function_gauss(dble(it-1)*DT-t0-tshift_src(isource),hdur_gaussian(isource)) !! FS FS replaced 5.d0*DT by hdur_gaussian(isource) (as should have been committed to devel branch by D.Komatitsch on 3 Sep 2015)
          endif
        else
          if (USE_RICKER_TIME_FUNCTION) then
            stf_pre_compute(isource) = comp_source_time_function_rickr(dble(it-1)*DT-t0-tshift_src(isource),hdur(isource))
      ...
      do isource = 1,NSOURCES
        ...
        if (USE_FORCE_POINT_SOURCE) then
          if (USE_RICKER_TIME_FUNCTION) then
            ...
            ! use a very small duration of 5*DT to mimic a Dirac in time
            stf_pre_compute(isource) = &
                 comp_source_time_function_gauss(dble(NSTEP-it)*DT-t0-tshift_src(isource),hdur_gaussian(isource)) !! FS FS replaced 5.d0*DT by hdur_gaussian(isource) (as should have been committed to devel branch by D.Komatitsch on 3 Sep 2015)
          endif
        else
          if (USE_RICKER_TIME_FUNCTION) then
            stf_pre_compute(isource) = comp_source_time_function_rickr(dble(NSTEP-it)*DT-t0-tshift_src(isource),hdur(isource))
